...
public class Enemy : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        GameObject nearest = GameManager.Instance.NearestPlayer(transform.position);

        if (nearest != null)
        {
            Vector3 playerDir = (nearest.transform.position - transform.position).normalized;

            Quaternion targetRotation = Quaternion.LookRotation(playerDir, Vector3.forward);
            transform.rotation = targetRotation;

            Vector3 newPosition = transform.position + playerDir * Time.fixedDeltaTime;
